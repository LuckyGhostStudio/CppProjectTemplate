...
一个用于快速构建项目的模板，适用于使用 Core/App 项目架构的 C++ 项目。其中包含两个项目：Core 和 App。[Premake](https://github.com/premake/premake-core)用于生成项目文件。
...
注：该项目模板修改自 [TheCherno](https://github.com/TheCherno/ProjectTemplate)。

## 分支说明
1. main - 主分支，从所有分支合并 (待定)
2. empty - 空分支，一个空的 C++ 项目模板
3. easyx - EasyX 分支，一个包含了 EasyX 的 C++ 项目模板

## 构建指南
1. 克隆该存储库的 `empty` 分支 `git clone -b empty --single-branch https://github.com/LuckyGhostStudio/CppProjectTemplate` 或使用 GitHub 上的 "Use this template" 按钮根据此模板快速设置自己的存储库。
